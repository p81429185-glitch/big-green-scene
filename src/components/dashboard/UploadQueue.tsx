...
const StatusIcon = memo(function StatusIcon({ status }: { status: QueueItem["status"] }) {
  switch (status) {
    case "done":
...
const QueueRow = memo(function QueueRow({
  ...
  return (
    <div className="px-3 py-2 space-y-1">
      ...
      {item.status === "error" && <p className="text-xs text-destructive break-words">{item.error}</p>}
      {item.status === "cancelled" && <p className="text-xs text-muted-foreground">Anulowano</p>}
    </div>
  );
});
      ...
      {minimized ? (
        ...
        <div className="max-h-72 overflow-y-auto">
          <div className="divide-y">
            {queue.map((item) => (
              <QueueRow key={item.id} item={item} onCancelItem={onCancelItem} />
            ))}
          </div>
        </div>
      )}
